  ...
  try {
    await req.user.populate({
      ...
      populate: [
        {
          path: 'users',
          select: 'id email name avatarPhoto',
        },
        {
          path: 'messages',
          populate: [{ path: 'file' }],
...
router.post('/', async (req, res) => {
  ...
  const alreadyExist = await ChatRoom.findOne({
    users: [req.user._id, req.body.receiver].sort(),
  })
  if (alreadyExist) {
  ...
  const chatroom = await new ChatRoom({
    users: [req.user._id.toString(), req.body.receiver].sort(),
  }).save()
